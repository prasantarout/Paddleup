...
import { getRulesReq } from '../redux/reducer/CmsReducer';
import { useDispatch, useSelector } from 'react-redux';
import RenderHTML from 'react-native-render-html';
import { KeyboardAwareScrollView } from 'react-native-keyboard-aware-scroll-view';

const OfficialRule = props => {
                ...
                <View style={[styles.cardWrapper]}>
                  <View
                    ...
                    <Text style={styles.ruleText}>
                      ...
                      {
                        CmsReducer?.getRulesRes?.data?.text1
                      }
                      {/* Official Rules and regulations{'\n'}of Pickleball (USA) */}
                    </Text>
                    <TouchableOpacity
                      onPress={() => {
                        Linking.openURL(CmsReducer?.getRulesRes?.data?.link_url1)
                      }}
                      style={styles.linkBtn}>
                      <Text style={styles.linkBtnText}>{CmsReducer?.getRulesRes?.data?.link_text1}</Text>
                    </TouchableOpacity>
                  </View>

                  <Separator
                    dividerStyle={{ zIndex: -1, borderTopColor: 'gray' }}
                  />
                  <View
                    style={[
                      styles.rowContainer,
                    ...
                    <Text style={styles.ruleText}>
                      ...
                      {
                        CmsReducer?.getRulesRes?.data?.text2
                      }
                    </Text>
                    <TouchableOpacity
                      onPress={() => {
                        Linking.openURL(CmsReducer?.getRulesRes?.data?.link_url2)
                      }}
                      style={styles.linkBtn}>
                      <Text style={styles.linkBtnText}>{CmsReducer?.getRulesRes?.data?.link_text2}</Text>
                    </TouchableOpacity>
                  </View>
                  <Separator
                    dividerStyle={{ zIndex: -1, borderTopColor: 'gray' }}
                  ...
                  <Text style={styles.rulesTitle}>
                    {
                      CmsReducer?.getRulesRes?.data?.section1_title
                    }
                  </Text>
                  {/* <Text style={styles.rulesPara}>
                    {CmsReducer?.getRulesRes?.data?.section2_title}
                  ...
                  <RenderHTML
                    contentWidth={'100%'}
                    source={{
                      html: CmsReducer?.getRulesRes?.data?.section1_content
                    }}
                    tagsStyles={{
                      p: styles.rulesPara
                    }}
                  />
                  <Text style={styles.rulesTitle}>{CmsReducer?.getRulesRes?.data?.section2_title}</Text>
                  <RenderHTML
                    contentWidth={'100%'}
                    source={{
                      html: CmsReducer?.getRulesRes?.data?.section2_content
                    }}
                    tagsStyles={{
                      p: styles.rulesPara
                    }}
