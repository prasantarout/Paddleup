...
const Home = props => {
  ...
  // in-app-purchase
  const [productLitst, setProductList] = useState(null);
 
 
  // SKUS
  const subscription = Platform.select({
    ios: ['paddletapYearlySub'],
  ...
  async function getSubscriptions() {
    let _outcome = await RNIap.initConnection().then();
    console.log('54 initialize', _outcome);

    try {
      // await RNIap.flushFailedPurchasesCachedAsPendingAndroid();
      const _subscriptions = await RNIap.getSubscriptions({skus: [subscription]});

      console.log(_subscriptions, 'products');
      console.log(_outcome, 'initialize');
      /** Filtering the SKU as per PARENT and SERVICE-PROVIDER **/
      if (Platform.OS === 'ios') {
        let _skuForUser = _subscriptions.map(item => {
          return item.productId;
        });
        setProductList(_skuForUser);
        setisVisible(false);
        console.log('_skuForUser', _skuForUser);
      } else {
        /** do for android **/
        console.log('in else android');

        let _skuForUser = _subscriptions.find(item => {
      ...
      console.log('err 76', JSON.stringify(err));
  ...
  function submit() {
    ...
    if (Platform.OS === 'android') {
      subName = 'your_product_id';
    } else {
      subName = 'paddletapYearlySub';
    }

    requestSubscriptionFromUser(subName);
  }

  // REQUEST SUBSCRIPTION
  async function requestSubscriptionFromUser(sku) {
    console.log('productId 109: ' + sku);
  ...
  async function acknowledgeSubscription(data) {
    ...
    try {
      let response = await RNIap.finishTransaction(data);
  ...
  useEffect(() => {
    ...
    purchaseUpdateSubscription = RNIap.purchaseUpdatedListener(purchase => {
      ...
      let obj = {
        ...
        transactionDate: purchase.transactionDate / 1000,
        productId: purchase.productId,
      };
      console.log(' 165 >>>>>>>>>.', obj);
      // dispatch(subcrIAPRequest(obj));
    });
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    // requestSubscription(myProducts);
                    // getSubData()
                    submit()
                  ...
                  <LinearGradient
                    ...
                    {/* {buyIsLoading === true ? */}
                      ...
                      <Text style={styles.subscriptionContinueButtonText}>{'Buy1'}</Text>
                      {/* } */}
